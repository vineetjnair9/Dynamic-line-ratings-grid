...
# Load DLR multiplier values
dlr_vals = pickle.load(open('dlr_vals_capped.pkl','rb'))
dlr_vals = pd.DataFrame(dlr_vals).iloc[:, 5:-(24-5)]  # Shift by 5 to convert UTC to Central time (ignoring daylight for now)
dlr_vals = dlr_vals.clip(lower=1)  # make sure not lower than 1
dlr_vals.columns = pd.date_range('2016-1-1','2016-12-31 23:00',freq='H')
dlr_vals.index = branch_t.index
...
def write_hour_case(dt):
    ...
    branch_t['rateA'] = texas.branch['rateA'] * dlr_vals[dt]  # Important step: update DLR ratings

    OUTPUT = 'input_cases_DLR/'
    OUTPUT_NAME = 'texas_' + dt.strftime('%Y-%m-%d_%H_DLR.m')
