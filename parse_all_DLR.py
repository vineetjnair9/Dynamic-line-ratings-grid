...
def parse_output(dt):
    ...
    lmps = pd.read_csv('outputs_DLR/lmps_{}_DLR.csv'.format(dt.strftime('%Y-%m-%d_%H')), index_col=0).sort_index().dropna(how='all')
    gens = pd.read_csv('outputs_DLR/gens_{}_DLR.csv'.format(dt.strftime('%Y-%m-%d_%H')), index_col=0).sort_index().dropna(how='all')
    branches = pd.read_csv('outputs_DLR/branches_{}_DLR.csv'.format(dt.strftime('%Y-%m-%d_%H'), index_col=0)).sort_index().dropna(how='all')
...
pg.to_csv('pg_DLR.csv')
lmp.to_csv('lmp_DLR.csv')
bdf.to_csv('bdf_DLR.csv')
